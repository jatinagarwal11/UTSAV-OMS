...
export default function AccountsDirectFactoryOrder() {
  ...
  const [customerName, setCustomerName] = useState('Factory Order');
  const [phone, setPhone] = useState('');
  const [address, setAddress] = useState('');
  const [notes, setNotes] = useState('Direct factory order created by Accounts');
  ...
  if (dataLoading) {
    return (
      <div className="animate-pulse">
        <div className="h-5 w-40 bg-[var(--border)] rounded mb-6" />
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-6">Direct Factory Order</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-lg p-5 space-y-3">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]">Order Info</h3>
            <Input
              label="Name"
              required
              value={customerName}
              onChange={(e) => setCustomerName(e.target.value)}
              placeholder="Factory order label"
            />
            <Input
              label="Phone"
              value={phone}
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-lg p-5">
            ...
            >
              {saving ? 'Creating...' : 'Create Factory Order'}
            </Button>
          </div>
        </div>
      </div>
